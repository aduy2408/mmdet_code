...
_UltralyticsMuSGD = MuSGD


@OPTIMIZERS.register_module()
class MuSGD(_UltralyticsMuSGD):
    """Framework adapter preserving Ultralytics MuSGD parameter semantics.

    MMEngine/MMCV pass parameter groups produced by their optimizer
    constructors. Ultralytics assigns 2D matrices and 4D convolution kernels
    to the Muon branch, while biases and normalization parameters stay on the
    SGD branch. Split incoming groups here so both frameworks use the same
    rule without changing the upstream optimizer update itself.
    """

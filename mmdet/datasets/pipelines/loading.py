...
class LoadTinyPersonImageFromFile(LoadImageFromFile):
    """Load a TinyPerson source image and crop its COCO ``corner`` tile.

    TinyPerson's corner annotations keep the source filename and store the
    tile window separately. Standard COCO loading would read the full source
    image while leaving tile-relative boxes unchanged.
    """

    def __call__(self, results):
        results = super().__call__(results)
        corner = results['img_info'].get('corner')
        if corner is None:
            return results

        x1, y1, x2, y2 = map(int, corner)
        image = results['img']
        if not (0 <= x1 < x2 <= image.shape[1] and
                0 <= y1 < y2 <= image.shape[0]):
            raise ValueError(
                f'Invalid TinyPerson corner {corner} for image shape '
                f'{image.shape[:2]}')
